...
function getInputFilename()
{
    ...
    if (getenv("TIMED") !== false) {
        ...
        $default = realpath(dirname($_SERVER['PHP_SELF'])) . '/' . basename(isTest() ? 'test.txt' : 'day17.txt');
...
PRESENT;
    ...
    {
        switch (rand(0, 3)) {
            case 0:echo getPresent($colour);
                break;
            case 1:echo getSanta($colour);
                break;
            case 2:echo getWreath($colour);
                break;
            case 3:echo getTree($colour);
                break;
        }
    }
